...
class SearchTreeNode:
    ...
    def __init__(self, hypothesis, meta, create_time, parent):
...
class NoPruneSynthesizer:
    ...
    def trace(self, example, cache=None):
        ...
        (init_hyp, init_meta), parser_state = self.parser.initialize_hyp_for_synthesizing(example)
        init_node = SearchTreeNode(init_hyp, init_meta, num_exec_steps, None)
    ...
    def continuations_of_node(self, node, parser_state, exec_time):
        ...
        new_nodes = [SearchTreeNode(p[0], p[1], exec_time, node) for p in progs]
